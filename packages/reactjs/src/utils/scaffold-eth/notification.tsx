...
const ENUM_STATUSES = {
  success: <CheckCircleIcon className="w-7 text-green-500" />,
  loading: <span className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin"></span>,
  error: <ExclamationCircleIcon className="w-7 text-red-500" />,
  info: <InformationCircleIcon className="w-7 text-blue-500" />,
  warning: <ExclamationTriangleIcon className="w-7 text-yellow-500" />,
};
...
const Notification = ({
  ...
  return toast.custom(
    t => (
      <div
        className={`flex flex-row bg-white items-start justify-between max-w-sm rounded-xl shadow-lg p-4 transform-gpu relative transition-all duration-500 ease-in-out space-x-2
        ${
      ...
      >
        ...
        <div className={`overflow-x-hidden break-words whitespace-pre-line flex-grow ${icon ? "mt-1" : ""}`}>
          {content}
        </div>
        <button
          className={`text-gray-500 hover:text-gray-700 ${icon ? "mt-1" : ""}`}
  ...
  return toast.custom(
    t => (
      <div
        ...
        } max-w-md w-full bg-slate-100 shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-black ring-opacity-5`}
      >
