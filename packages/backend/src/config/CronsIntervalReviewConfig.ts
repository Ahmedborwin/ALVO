...
const StartIntervalReview = () => {
  const abi = chainHabits["84532"]["ChainHabits"].abi;
  const address = chainHabits["84532"]["ChainHabits"].address;
  const providerUrl =
    "https://eth-sepolia.g.alchemy.com/v2/U4KPgXJi3FAILfXVYloxhngoXfSLZnER";
  try {
    const runDailyNight = async () => {
      ...
      const Address_liveChallenges =
        await chainHabitsContract.getLiveChallenges(userAddressList);
    ...
    const task = cron.schedule(cronSchedule, runDailyNight);
    task.start();
  } catch (error) {
    console.error(error);
  }
...
export default StartIntervalReview;

// -------------------------OLD cron job from NEXT-JS---------------------------
